...
export function executeTests() {
  ...
  const numExpr: Array<[string, ...any[]]> = [
    /* number expressions */
    ['1+1', 2],
    ['floor( -3.1415 )', -4],
    ['-5 mod -2', -1],
  ...
  const evalNodeSet = (ex: string, ctx: XSLTContext) => {
    const e = ctx.eval(ex);
    return gatherNodes(e);
  };
  ...
  const doTestEvalDom = (
    ...
    expect(ns.length).to.equal(1, page);

    ns = evalNodeSet(page, ctx1);
    expect(ns.length).to.equal(1, page);

    ns = evalNodeSet(slashPage, ctx);
    expect(ns.length).to.equal(1, slashPage);

    ns = evalNodeSet(slashPage, ctx1);
    expect(ns.length).to.equal(1, slashPage);

    expect(evalNodeSet('/', ctx).length).to.equal(1, '/');
    expect(evalNodeSet('/', ctx1).length).to.equal(1, '/');

    expect(evalNodeSet('/', ctx)[0].nodeName).to.equal('#document', '/');
    expect(evalNodeSet('/', ctx1)[0].nodeName).to.equal('#document', '/');

    expect(evalNodeSet(slashPage, ctx)[0].nodeName).to.equal(page, slashPage);
    expect(evalNodeSet(slashPage, ctx1)[0].nodeName).to.equal(page, slashPage);

    let n = evalNodeSet(slashPageLocationAtLat, ctx)[0];
    expect(n.nodeName).to.equal(lat, slashPageLocationAtLat);
    expect(n.nodeValue).to.equal(latValue, slashPageLocationAtLat);
    ...
    expect(n.nodeName).to.equal(lon, slashPageLocationAtLon);
    expect(n.nodeValue).to.equal(lonValue, slashPageLocationAtLon);
  ...
  describe('xpath', () => {
    it('can parse the xpaths', () => {
      const nsResolvder = evaluator.createNSResolver();
      // tslint:disable-next-line:prefer-for-of
      for (let i = 0; i < expr.length; ++i) {
        const expression = evaluator.createExpression(expr[i], nsResolvder);
    ...
    it('can evaluate variables on a HTML context', () => {
      ...
      for (const e of numExpr) {
        ...
        try {
          ...
          // expect(ex).to.equal(e[1], ex);
          continue;
        }
        if (typeof e[1] === 'number') {
          expect(result.numberValue).to.equal(e[1], e[0]);
        } else if (typeof e[1] === 'string') {
          expect(result.stringValue).to.equal(e[1], e[0]);
        } else if (typeof e[1] === 'boolean') {
          expect(result.booleanValue).to.equal(e[1], e[0]);
        }
      }
    });
    ...
    it('can evaluate axis on a context', () => {
      ...
      const axisTests: Array<[string, any]> = [
    ...
    it('can handle whitespace', () => {
      ...
      expect(value).to.equal(' Here is some funky  text that contains  spaces and stuff ');
    });
    ...
    //   for (const test of tsts) {
    //     assert.equal(xpathParse(test[0]).steps[1].hasPositionalPredicate, test[1], test[0]);
    //   }
    // });
    ...
    it('returns on first match', () => {
      ...
      for (const test of tests) {
        ...
        const normalResults = ctx.eval(ex);
        const normalNodes = gatherNodes(normalResults);
        expect(normalNodes.length).to.equal(test[1], `normal results count: ${test[0]}`);

        ctx.setReturnOnFirstMatch(true);
        const firstMatchResults = ctx.eval(ex, 9); // FIRST_ORDERED_NODE_TYPE
        const firstMatchNodes = gatherNodes(firstMatchResults);
        expect(firstMatchNodes.length).to.equal(1, `first match results count: ${test[0]}`);

        expect(normalNodes[0]).to.equal(
          firstMatchNodes[0],
